...
def optimize_hyperparameters(X_train, Y_train, X_test, Y_test, n_trials=150):
    ...
    base_weights = len(Y_train_np) / (len(class_counts) * class_counts)
    ...
    def objective(trial):
        ...
        params = {
            ...
            # Optimize class weights individually
            'weight_class_0': trial.suggest_float('weight_class_0', base_weights[0] * 0.5, base_weights[0] * 2.0),
            'weight_class_1': trial.suggest_float('weight_class_1', base_weights[1] * 0.5, base_weights[1] * 2.0),
            'weight_class_2': trial.suggest_float('weight_class_2', base_weights[2] * 0.5, base_weights[2] * 2.0),
            'weight_class_3': trial.suggest_float('weight_class_3', base_weights[3] * 0.5, base_weights[3] * 3.0),  # Wider range for rare class
        }
        
        # Extract weights and remove from params
        class_weights = {
            0: params.pop('weight_class_0'),
            1: params.pop('weight_class_1'), 
            2: params.pop('weight_class_2'),
            3: params.pop('weight_class_3')
        }
        ...
        # Apply class weights using sample_weight
        ...
        # Use macro F1-score as optimization target
    ...
    print("Multi-class optimization completed!")
...
def plot_optimization_results(study):
    """Hyperparam optimisation visualizations"""
    ...
    try:
        ...
        if len(top_params) >= 2:
            ...
            plt.colorbar(scatter, ax=ax1, label='Loss (mlogloss)')
...
# Visualisations
def create_vis(best_model, X_train, Y_train, Y_test, X_test, best_lr):
    ...
    axes1[0, 0].plot(x_axis, results['validation_0']['mlogloss'], label='Train', linewidth=2, color='blue')
    axes1[0, 0].plot(x_axis, results['validation_1']['mlogloss'], label='Test', linewidth=2, color='red')
    axes1[0, 0].legend()
    ...
    # Accuracy over time
    train_accuracy = [1 - x for x in results['validation_0']['mlogloss']]
    test_accuracy = [1 - x for x in results['validation_1']['mlogloss']]
...
def main():
    ...
    X_train, Y_train, X_test, Y_test = load_data(file_path_train, file_path_test)
    
    # Optuna optimise hyperparameters
    n_trials = int(input("How many optimization trials? (default 150): ") or 150)
    study = optimize_hyperparameters(X_train, Y_train, X_test, Y_test, n_trials)
    ...
    create_vis(best_model, X_train, Y_train, Y_test, X_test, best_lr)
