...
def optimize_hyperparameters(X_train, Y_train, X_test, Y_test, n_trials=150, study_name='xgboost_study', n_jobs=-1, output_dir='results'):
    ...
    logger.info(f"Base class weights: {base_weights}")
    ...
    def objective(trial):
        ...
        params = {
            ...
            # With this more controlled approach:
            'weight_ratio_0': trial.suggest_float('weight_ratio_0', 0.7, 1.3, log=True),  # Close to 1
            'weight_ratio_1': trial.suggest_float('weight_ratio_1', 0.8, 1.2, log=True),  # Even tighter
            'weight_ratio_2': trial.suggest_float('weight_ratio_2', 0.7, 1.3, log=True),
        ...
        class_weights = {
            0: base_weights[0] * params.pop('weight_ratio_0'),
            1: base_weights[1] * params.pop('weight_ratio_1'),
            2: base_weights[2] * params.pop('weight_ratio_2')
        }
        ...
        cv_scores = []  # Store scores from each fold
        ...
        for train_idx, val_idx in cv.split(X_train_np, Y_train_np):
            ...
            # Predict on the CV not on the test data
            preds = model.predict(X_cv_val)
            
            # Calculate score on cv
            bal_acc = balanced_accuracy_score(y_cv_val, preds)
