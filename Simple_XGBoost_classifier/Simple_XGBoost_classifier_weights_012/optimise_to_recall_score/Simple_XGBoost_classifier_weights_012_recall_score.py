...
def prepare_data(X_train, Y_train, X_test, Y_test, scale_data=True):
...
def optimize_hyperparameters(X_train, Y_train, X_test, Y_test, n_trials=150):
    print(f"Starting Optuna hyperparameter optimization for recall with {n_trials} trials...")
    ...
    print(f"Class percentages: {class_counts / total_samples}")
    ...
    def objective(trial):
        params = {
            ...
            # Aggresive weighting optimisation
            'weight_class_0': trial.suggest_float('weight_class_0', 1.0, 4.0), 
            'weight_class_1': trial.suggest_float('weight_class_1', 0.5, 1.0),  
            'weight_class_2': trial.suggest_float('weight_class_2', 1.0, 3.0),
            
            # Scale_pos_weight equivalent for multi-class
        ...
        # Optimisation to both reacall and precision
        combined_score = (0.7 * recall) + (0.3 * precision)
    ...
    print(f"Best target recall: {-study.best_value:.4f}")
