...
validations=["Q-mean", "f1"]
study_names=["4deep","6deep","8deep","10deep"]
# --- Traindata inlezen ---
...
for validation in validations:
    for study_name in study_names:    
        ...
        print("Best weights: "+study_name+validation, study.best_params)
        print("Best macro-F1: "+study_name+validation, study.best_value)
        ...
        for lr_idx, lr in enumerate(learningrates):
            model = SimpleNN(input_dim=X_train.shape[1], hidden_dim=hiddendim)  # <-- nieuw model
        ...
        plt.title(f'Normalized confusion Matrix (lr={lr}) ' +study_name+validation)
        plt.savefig("./results/normalizedconfusionmatrix "+study_name+validation)
        ...
        disp.plot(cmap=plt.cm.Blues)
        plt.title(f'Confusion Matrix (lr={lr}) ' +study_name+validation)
        plt.savefig("./results/confusionmatrix "+study_name+validation)
        ...
        plt.xlabel("epoch")
        plt.ylabel("accuracy")
        plt.title("NN Accuracy "+study_name+validation)
        plt.savefig("./results/accuracy "+study_name+validation)
        ...
        plt.xlabel("epoch")
        plt.ylabel("loss")
        plt.title("Loss per epoch "+study_name+validation)
        plt.legend()
        plt.savefig("./results/loss "+study_name+validation)
        ...
        plt.xlabel("epoch")
        plt.ylabel("recall")
        plt.title(f"Recall per class (lr={learningrates[0]}) "+study_name+validation)
        plt.legend()
    
        plt.savefig("./results/recall "+study_name+validation)
